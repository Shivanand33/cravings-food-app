...
const UserDashboard = () => {
    ...
    return (
      <div className="min-h-[calc(100vh-64px)] bg-orange-50/50 flex items-center justify-center p-4 font-sans">
        <div className="bg-white rounded-3xl shadow-xl border border-red-100 p-8 max-w-md w-full text-center space-y-4">
          ...
          <h2 className="text-2xl font-extrabold text-gray-800">Access Denied</h2>
          <p className="text-sm text-gray-500">
            You are currently not logged in as a Customer. Please sign in with a customer account.
          </p>
          <button
  ...
  return (
    <div className="h-[calc(100vh-64px)] bg-slate-50 flex overflow-hidden font-sans relative">
      ...
      {/* 🟢 Sidebar Container */}
      <aside
        className={`fixed lg:static top-0 bottom-0 left-0 bg-white border-r border-gray-200 transition-all duration-300 z-40 flex flex-col ${isMobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
        ...
        {/* Top Header Bar */}
        <header className="h-16 bg-white border-b border-gray-200 px-4 sm:px-6 flex items-center justify-between shrink-0 shadow-xs">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg sm:text-xl font-extrabold text-slate-800 capitalize tracking-tight">
                {tabTitles[active] || "Dashboard"}
              </h1>
              <p className="text-xs text-gray-400 hidden sm:block">
                Manage your food orders, profile, and payment history
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="text-right hidden sm:block">
              ...
              <span className="text-[10px] bg-orange-100 text-[#FF5722] px-2 py-0.5 rounded-full font-semibold uppercase">
                Customer
              </span>
            </div>
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-[#FF5722] to-amber-400 text-white font-bold flex items-center justify-center shadow-md overflow-hidden shrink-0 border border-orange-200">
              {userPhoto ? (
